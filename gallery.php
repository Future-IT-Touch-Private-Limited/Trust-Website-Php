...
.gallery-col img, iframe {
  ...
  <div class="section section-padding">
    <div class="container">
      ...
      <div class="row mb-5">
        <div class="gallery-title">Album's</div>
          <?php
      ...
      if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
          $catID = $row['id']; 
           $countsql = "SELECT count('id') as total_images FROM `our_rooms` WHERE  `category_id` = '$catID' ";
              $countresult = mysqli_query($conn, $countsql);
                $count_row = mysqli_fetch_assoc($countresult);

            $imagesql = "SELECT `our_rooms_image`, `our_video_url` FROM `our_rooms` WHERE  `category_id` = '$catID' ORDER BY `id` LIMIT 1";
              $imageresult = mysqli_query($conn, $imagesql);
                $image_data = mysqli_fetch_assoc($imageresult);

                //print_r($image_data);
                if($count_row['total_images'] > 0){
          ?>
          <div class="col-md-4 col-xs-12 gallery-col">
            
              <?php if($image_data['our_rooms_image'] != NULL){ ?>
              <img class="img-thumbnail" src="images/<?php echo $image_data['our_rooms_image'] ?>">
            <?php } else { ?>
              <iframe src="<?= $image_data['our_video_url']; ?>"></iframe>
            <?php } ?>
            <a href="gallery-images.php?catid=<?= $row['id']; ?>">
                <h5><?php echo $row['name'] ?></h5>
                <span class="item-cou"><?=  $count_row['total_images'];?> Items</span>
            </a>
         </div>
        <?php } } } ?>
       </div> 
